...
def build_agent_executor(cfg: dict[str, Any]):
    ...
    system_prompt = (
        ...
        "For canned multi-step workflows use run_skill('demo', ...): when asked to "
        "scan the WHOLE panorama / every subsection / every backend, make ONE call — "
        "run_skill(\"demo\", '{\"name\": \"panorama_scan\"}') — instead of looping "
        "PTZ tools yourself (it slices headings, runs each backend, and returns a "
        "rollup). Other demos: edge_gateway_preflight, "
        "ptz_multimodel_scientific_survey, wildfire_smoke_patrol, "
        "aves_biodiversity_scan, land_cover_agriculture_scene. "
        "To find/onboard a networked sensor or PTZ camera (e.g. 'find the camera on "
